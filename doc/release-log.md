# 发布日志

## 2.25.0

- 缺陷：修复目标为 kafka 时，自定义字段无法添加问题。
- 优化：减少 Mysql serverId 冲突问题，用 UUID 替代端口号

## 2.24.0

- 重构：引入统一重试机制。
- 缺陷：修复因大小写匹配导致 Mysql DDL 事件过滤失效问题。
- 缺陷：修复 mysql 到 sql server 同步 自增字段属性丢失问题。
- 缺陷：修正移除同步任务中 meta 缺失问题，进行忽略，能移除的尽量移除。
- 缺陷：错误队列批量执行统计错误问题修正。
- 缺陷：修复集成测试过时问题
- 优化：消除界面僵住风险（一直显示拼命加载）
- 优化：字段映射结构简化，提高可维护性。

## 2.23.1

- 缺陷：修复 mysql 增量对非标准编码格式的支持（有问题的任务、字段映射需重新保存）
- 缺陷：修复因特殊类型无法加载错误队列问题
- 缺陷：修复 mysql 增量模式 set 类型异常问题
- 优化：增量异常数据处理优化

## 2.23.0

- 功能：mysql 增量增加对非标准编码格式的支持
- 缺陷：kafka 数据日期被替换问题
- 缺陷：mysql Time 类型无法序列化问题。
- 缺陷: 修正前端大小写问题

## 2.22.1

- 缺陷：修复 sql server 任务无法启动问题
- 缺陷：修复异常数据重新同步异常

## 2.22.0

- 功能：实现映射与目标字段的比对，并提供清理功能。
- 缺陷：修复批量转换配置按选择能力。
- 重构：统一大小写相关逻辑，减少后续维护成本

## 2.21.2

- 缺陷：修复字段重复问题，包括主键和自定义字段。

## 2.21.1

- 功能：批量设置主键能力
- 优化：统一字段映射处理方式
- 优化：合并字段比对代码

## 2.20.2

- 缺陷：解决kafka 类型转换问题
- 缺陷：去除表组编辑时对 kafka 的建表检测功能(OK)
- 缺陷：修复数据表改名报错“目标表不存在”问题
- 优化：复用缺表检测逻辑

## 2.20.1

- 功能：全量异常数据输出到错误队列
- 功能：对主键顺序窗口增加删除功能
- 缺陷：重置将错误队列一起清理。
- 缺陷：修复字段映射主键发生变化，主键顺序窗口没有做相应的调整。
- 缺陷：修复目标表未创建，同时发生主键变更的情况下，无法保存问题。
- 缺陷：修正建表逻辑，优先以选择的字段和主键为依据而不是以源表为依据进行建表。
- 缺陷：修正调整主键时，sql server 找不到主键问题。
- 缺陷：修正建表后保存逻辑部分信息丢失问题。
- 缺陷：修复主键顺序无法调整问题
- 优化：先保存自定义字段再执行主键变更。

## 2.19.9

- 批量添加配置功能增加给配置字段设置成临时主键功能

## 2.19.8

- sql server ct
  - 修正：IntID numeric(18,0) IDENTITY(1,1) NOT NULL, 类型解析异常
  - 修正：decimal 类型 转换问题。

## 2.19.7

- sql server ct
  - 解决 null 字段无法插入或更新问题， 解决方案：用 right join 替代 left join + union 补 D 操作(OK)
  - 同一批次中的同一数据，既有I，又有U，但 ChangeTable 只有U，造成少数据。解决方法将U变为I（OK）
  - 写数据时主键冲突问题，因异步写入合并多个查询批次导致，去重解决（OK）
  - 管理员支持一键为多对映射添加自定义字段

## 2.19.6
- 优化：修改SQL server超时时间从60秒改成五分钟
- 优化：SQL server优化ct获取时间间隔，优化在源端获取数据时依据主键去重操作，优化传输性能
- 优化：主键差异检测不支持大小写检测
- 修正：修复SQL server数据库CT模式下向kafka同步数据的时候数据库名丢失问题

## 2.19.5
- 修正，支持mysql枚举类型向varchar和int类型进行转换
- 修正：修复自定义字段类型比如VARCHAR(64)转换成真实字段类型失效bug

## 2.19.4
- 修正：修复了SQL serverCT删除字段列表导致了表字段和表数据不匹配bug

## 2.19.3
- 修正：修复自定义字段类型时smallInt类型同步失败bug
- 修正：同步到kafka时间戳形式改成时间格式
- 优化：主键变化弹窗检测机制

## 2.19.2
- 修正：SQL server自定义字段类型同步失败问题
- 修正：自定义字段二次保存丢失fieldMetadata问题以及还会二次执行创建自定义字段ddl问题
- 修正：添加任务的时候丢失mapping问题
- 优化：添加目标表冲下拉框调整为弹窗，并支持多表检索

## 2.19.1

- 修正：sql server 主键调整语法错误
- 修正：修复增量在连接中断会丢数据的问题。
- 修正：增量发生异常后无法退出问题。
- 优化：自定义字段和主键操作的异常入库
- 优化：增量退出机制使用独立停止机制。与连接中断解耦。

## 2.19

- 新功能: 帮助文档功能
- 新功能: 展示字段差异，并且支持修改目标与源对齐功能
- 修正：从根上修正主键顺序问题
- 修正：增加自定义字段异常处理

## 2.18

- 新功能：增加了指定数据表重新同步功能
- 新功能：增加清空数据库功能
- 新功能：增加自定义字段的移除功能（仅支持删除未保存的自定义字段，纯前端实现）
- 新功能：任务复制可变更数据源
- 新功能：可自定义主键及调整主键顺序，同时调整建表主键顺序按自定义主键顺序实施。
- 修正：当 mysql 数据表中有 enum，set 字段进行建表时出错。
- 修正：修复用户修改密码导致用户组丢失bug

## 2.17

- 功能：提供基本数据清洗功能，仅测试默认值。
- 功能：权限功能增加用户组来进行关联用户和任务组
- 缺陷：修复bug数据源 关联的相关任务运行状态显示不对， 均显示未执行
- 优化：调整卡片布局的每个卡片间隔使其适应不同的页面窗口

## 2.16

- 功能：实现用户按项目组授权
- 缺陷：修正任务列表时间问题
- 优化：任务具体页面增加启停功能
- 优化：任务面板布局调整。

## 2.15.1

- 新功能：目标端 kafka 支持 DDL 写入
- 缺陷：阻止 kafka 建表行为
- 缺陷：修复 sql server 新建表直接同步自增字段报错问题。

## 2.14

- 优化：重构失败重做并增加全部重试功能。注意：不兼容此版本之前的错误数据。
- 缺陷：修复并发计数问题。
- 缺陷：修复目标字段叠加问题。
- 缺陷：修复 sql server BOOLEAN 类型对 BIT 的支持。

## 2.13.1

- 优化：解决计数过于频繁写库问题。
- 缺陷：解决不能立即停止 server ct 增量任务的问题。 
- 界面缺陷：解决任务按钮乱跳问题。

## 2.13

- sql server CT优化：
  - 避免队列溢出丢数据风险。
  - 改用流式查询，解决大事务过大无法同步问题；
  - 移除DDL队列。
  - 简化 version 更新机制。
  - 避免重复构建主键和列信息。
  - 简化快照保存方式。
  - 修复混合模式快照无效问题
- 优化：统一增量数据入队处理，提高可维护性。
- 缺陷：修复 sql server max 字段无法在 mysql 端建表问题。
- 优化：优化混合模式快照处理方式
- 功能调整：将原来的任务启动过后就不能编辑修改成运行中不能编辑
- 界面：
  - 任务页面增加对应的日志数据页面。
  - 将任务同步类型移动到添加任务界面
  - 将表同步进度列表放到概括标签页中
  - 列表数量逻辑重构，新增同步状态，速度，移除全量同步进度
  - 修复添加映射不会在映射列表刷新 bug
  - 调整卡片布局

## 2.12.2

- 新功能：可看每个表的同步状态。
- 新功能：实现了数据源和任务的搜索功能。
- 重大缺陷：修复反复启动丢数据问题。
- 废弃配置列表：dbsyncer.parser.table.group.*
- 优化 sql server ct 查询性能
- 界面优化：连接页面调整，任务卡片调整、任务编辑界面调整。
- 重构：快照保存机制。
- 缺陷：修复刷新表按钮无法同步数据源表最新的数据的 bug
- 缺陷：修复各个界面的返回按钮只能返回到首页的bug
- 其它优化


## 2.11.4

- 新功能：任务一旦开始，编辑功能受限。（OK）
- 优化：前端回退功能优化。（OK）
- 缺陷：更改快照保存机制，避免丢失数据（OK）
- 缺陷：修正任务编辑界面交替显示问题(OK)
- 缺陷：解决 sql server CT 增量不同步问题(Ok)
- 缺陷：修复 mysql 增量同步获取表字段不准问题。
- 严重缺陷：修复增量同步丢数据问题（字段匹配问题）（OK）
- 缺陷：kafka 连接异常，不能创建、编辑任务问题。（OK）
- 缺陷：修复任务中的连接丢失导致任务页面无法加载的问题。(OK)
- 缺陷：修复监控页因 Meta 缺失 Mapping 导致无法加载问题。(OK)
- 优化：新建、编辑 Mapping 时检查连接是否存在。(OK)
- 优化：先创建 Mapping 后创建 Meta。(OK)
- 优化：避免 sql 生成时加载无效连接。(OK)
- 缺陷：修复启动日志各个配置计数不准确问题。（OK）

## 2.10

- 新功能：可以从 binlog 的任意位置开始，
- 优化快照更新机制。
- 缺陷：修复mysql binlog json 字段解析错误

## 2.9.1

- 新功能：任务增加列表方式。
- 新功能：提供任务查询功能。
- 功能调整：数据源独立管理。
- 缺陷：修复 sql server 参数达 2100 个问题。
- 缺陷：修复 sql server CT 模式下无法删除数据问题。
- 缺陷：修复 sql server 同构同步自增主键无法建表问题。
- 性能优化：sql server CT 模式下避免 DML 每次都查主键（getPrimaryKeys）。

## 2.8.0

- 新功能：基于 CT(change tracing) 实现 mssql ddl(DML 驱动)，之所以不用 cdc 主要是 sql server
  端的压力，同时消除表结构变化这种自身缺陷影响同步数据正确性问题.(OK)
- 新功能：对连接器增加“是否支持DDL写入”功能，以忽略 kafka 等非标准数据库的建表检测动作。（OK）
- 缺陷修复：不同线程之间串数据问题。（command, table）(ok)
- 缺陷修复：修复无法重建与 kaka 的表映射关系。（OK）
- 性能优化：使用性能更高的记录数查询机制。(OK)
- 结构优化：字段映射主要参考原表，简化操作过程。（OK）
- 优化：对无主键数据表进行过滤处理，并形成错误日志。(OK)

## 2.7.0

- 新功能：目标表缺失感知与创建。
- 新功能：支持目标表重命名。
- 新功能：Mysql 支持DDL 支持。增加了相应的开关：加列、删列、改列。注意，忽略了缺省值的处理，因为一是每种库缺省值的函数表达非常不同，比如时间戳，较为复杂；二是不影响同步结果。
    - 优化：重构中间标准化类型, 并对标准类型进行了扩展(XML,JSON,ENUM,SET,TEXT,UNSIGNED,unicode,UUID,BLOB,geometry),移除
      ARRAY 类型。
- 结构优化：大幅度改善不同数据库的 schema 解析工作的抽象，提供其可维护能力。
- 优化：停止同时使用两套json框架，移除 fastjson 的使用，给维护减压。
- UE 优化：任务面板布局重构。(OK)

## 2.6.0

- 性能优化：重构同步覆盖，使用 upsert 技术替代现有的非批量、IO繁重的方式。
- 性能优化：大幅度提升sql server 写入慢问题。
- 性能优化: 解决重复加工 sql 指令列表问题。（OK）
- UE 优化：优化分组列表的显示。
- UE 优化：解决上下文弹出菜单因自动刷新而消失的问题。(OK)
- bug fixed: 解决 mysql, mssql 发现的类型转化问题。
- bug fixed: 全部表移除时计数统计不为0问题。(OK)

## 2.5.0

- 新功能：增加 Sql Server 的异构同步输出机制。
- 结构优化：优化字段映射处理机制
- 性能优化：mysql, sql server 使用流式读取数据，大表性能提升数倍，支持断点续传。
- 性能优化：支持多表并发处理。
- 性能优化：减少从缓存读取数据的任务线程的竞争数量
- 性能优化：同步过程错误信息会累加并不断膨胀，对性能、和系统稳定性影响较大。
- 调整：直接使用 JSqlPhase，废弃对 JSqlPhase 的包装（2万行）
- 结构优化：将 DQL 类连接器重构，简化代码。
- 结构优化：使用 sql template 技术 替代 sql builder 技术，简化 sql 生成的层次结构。(OK)；
- 结构优化：用 Meta 替代 Task 类，解决 Task 在多线程环境下导致的任务有时无法关停问题。
- 结构优化：对 TableGroup 和 Mapping 进行了功能扩展，以降低主线程序的复杂度。
- 优化：meta 和 tableGroup 的名字更具有可读性和意义。
- 优化：使用左右引号而非单一引号，解决 SQL server 引入的双重复杂度。
- 优化: tableGroup 不需要持久化 sql（OK）
- bug fixed: 编辑导致状态重置问题。(OK)
- bug fixed：重置计数为历史数量问题。(OK)
- bug fixed: 复制的任务，数据总数没有计算问题。（OK）

## 2.4.0

- 新功能：全量+增量混合模式实现(Ok)
- 新功能：mapping 的扩展参数可下拉选择(OK)
- 新功能：为“驱动”提供异常信息的展示。（OK）
- 新功能：增加重置状态（仅适用于混合模式）(ok)
- 新功能：增加了异常状态。（OK）
- 性能优化：提升 mysql 源端解析性能。(OK)
- 优化：取消 closeEvent 事件的使用（OK）
- 优化：移除 AbstractPuller 类（OK）
- 优化: Meta 集成持久化逻辑，以简化维护。（OK）
- 优化：puller 接口 优化。(OK)
- 优化：移除 mete stopping 状态（OK）

## 2.3.1

- 新功能：字段全选既过滤功能
- disable kafka group.id check
- bug fixed: 以 kafka 为目标的新同步任务，配置时无法保存。
- bug fixed: pk missed when add mapping

## 2.2.0

- bug fixed: max 线程数小于 core 线程数，线程池异常问题。
- 驱动：新增扩展参数编辑能力。
- 扩展参数：可覆盖 kafka 的缺省 topic 设置。
- 移除 kafka 的字段定义设置。

## 2.1.1

性能优化：RefreshOffsetEvent，FullRefreshEvent 使用回调机制替代 spring 事件机制

## 2.1.0 2025年8月5日

- bug fixed: 数据同步到 kafka ，kafka接收不到消息
- 支持 bit 数据类型
- ui: when target table is null then use source table name
- ui: when target field is null then use source field name
- kafka use one topic to receive all tables
- kafka 输出格式兼容 DataPipeline
